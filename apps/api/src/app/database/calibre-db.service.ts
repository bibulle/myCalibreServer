...
@Injectable()
export class CalibreDbService {
  private readonly logger = new Logger(CalibreDbService.name);
  ...
  constructor(private readonly _configService: ConfigService) {
    // CalibreDbService.CALIBRE_DIR = this._configService.get('PATH_BOOKS') || path.resolve(`${__dirname}/../../../data/calibre`);
    // CalibreDbService.DB_FILE = `${CalibreDbService.CALIBRE_DIR}/metadata.db`;

    // this._getDb();
    // this.getDbDate();
    // this.logger.debug(`Opening '${CalibreDbService.DB_FILE}'`);
    ...
    //     this.logger.error(`Cannot open calibre Db : '${CalibreDbService.DB_FILE}'`);
    //   });
  }

  // private _getDb(): Database {
  //   if (!this._db) {
  //     this._db = new Database(CalibreDbService.DB_FILE);
  //   }
  //   return this._db;
  // }
  ...
  //     fs.stat(CalibreDbService.DB_FILE, (err, stats) => {
  //       if (err) {
  //         console.log(err);
  //         reject(err);
  //       } else {
