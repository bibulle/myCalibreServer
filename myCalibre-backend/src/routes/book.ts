                           ...
                           if (data && (data.length != 0)) {
                             ...
                             fs.stat(fullPath, (err) => {
                               if (err) {
                                 ...
                                 // Send mail
                                 //var urlSmtp = `smtp${Configuration.getInstance().smtp_encryption == "SSL" ? 's' : ''}://${Configuration.getInstance().smtp_user_name}:${Configuration.getInstance().smtp_password}@${Configuration.getInstance().smtp_server_name}:${Configuration.getInstance().smtp_port}`;
                                 const urlSmtp = `smtp${Configuration.getInstance().smtp_encryption == "SSL" ? 's' : ''}://${Configuration.getInstance().smtp_user_name.replace('@', '%40')}:${Configuration.getInstance().smtp_password}@${Configuration.getInstance().smtp_server_name}`;
                                 const transporter = nodemailer.createTransport(urlSmtp);

                                 transporter.sendMail({
                                   from: `<${Configuration.getInstance().smtp_user_name}>`,
                                   to: `${mail}`,
                                   subject: 'My books',
