...
import {Directive, OnDestroy, Input, ElementRef, NgZone, NgModule, HostBinding, OnInit, HostListener} from '@angular/core';
...
export class MatPeekabooDirective implements OnInit, OnDestroy {

  static SIZES: string[] = ['(max-width: 599px)', '(min-width: 600px) and (max-width: 959px)', '(min-width: 960px) and (max-width: 1279px)', '(min-width: 1280px) and (max-width: 1919px)', '(min-width: 1920px)'];
   ...
   */
  evaluate(): number {
    let top = this._scroller ? MatPeekabooDirective.scrollTop : MatPeekabooDirective.scrollTop();
